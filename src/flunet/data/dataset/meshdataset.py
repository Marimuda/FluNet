...
class MeshDataset(Dataset):
    def __init__(self, hparams, split="train", transform=None, pre_transform=None):
        self.split = split
        self.hparams = hparams
        self.dataset_name = hparams.dataset_name
        super().__init__(self.hparams.data_dir, transform, pre_transform)
    ...
    def triangles_to_edges(self, faces):
        ...
        edges = np.concatenate([faces[:, 0:2], faces[:, 1:3], np.stack([faces[:, 2], faces[:, 0]], axis=1)], axis=0)
        ...
        return np.stack(
            [np.concatenate([senders, receivers], axis=0), np.concatenate([receivers, senders], axis=0)], axis=0
        )
    ...
    def _parse(self, proto, meta):
        ...
        for key, field in meta["features"].items():
            data = tf.io.decode_raw(features[key].values, out_type=field["dtype"])
    ...
    def process(self):
        ...
        for idx, data in enumerate(ds):
            ...
            torch.save(
                data_transformed, os.path.join(self.processed_dir, self.split, f"data_{idx}.pt")
            )  # (#node, traj, feat_dim)
