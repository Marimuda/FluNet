...
def get_activation_and_regularization_layers(in_features: int, regularization_config: ConfigDict) -> nn.ModuleList:
    ...
    if latent_normalization is not None:
        assert latent_normalization in [
            "LayerNorm",
            "GraphNorm",
            "InstanceNorm",
            "BatchNorm",
        ...
        if latent_normalization in ["BatchNorm"]:
            new_modules.append(SaveBatchNorm1d(num_features=in_features))  # Implement
        else:
            norm_layer = getattr(nn, latent_normalization)
            new_modules.append(norm_layer(in_features))
...
def get_layer_size_layout(max_neurons: int, num_layers: int, network_shape: str = "block") -> np.ndarray:
    ...
    if network_shape in ["=", "==", "block"]:
        ...
        return np.array([int(np.maximum(1, max_neurons // (2**distance))) for distance in range(num_layers)])
    elif network_shape in ["<", "expanding"]:
        return np.array(
            [int(np.maximum(1, max_neurons // (2**current_layer))) for current_layer in reversed(range(num_layers))]
        )
    elif network_shape in ["><", "hourglass"]:
        return np.array(
            [
                int(np.maximum(1, max_neurons // (2**distance)))
        ...
        return np.array(
            [
                int(np.maximum(1, max_neurons // (2**distance)))
                for distance in list(reversed(range((num_layers + 1) // 2)))
                + list(range((num_layers + 1) // 2))[num_layers % 2 :]
            ]
        )
    else:
        raise ValueError(
            "Unknown network_shape '{}'. Eligible shapes are = ('block'), > ('contracting'), "
