...
class GNNBase(AbstractGNNBase):
    ...
    def __init__(
        ...
        num_node_features = list(in_node_features.values())[0] + NodeType.SIZE
        ...
        if self._use_global_features:
            self._global_input_embedding = LinearEmbedding(
                in_features=in_global_features, out_features=latent_dimension
            )

        # create message passing stack
        self._message_passing_stack = HomoMessagePassingStack(
    ...
    def forward(
        ...
        node_features, edge_features, global_features = self._message_passing_stack(
            node_features=node_features.view(-1, node_features.shape[-1]),
            edge_index=edge_index,
            edge_features=edge_features.view(-1, edge_features.shape[-1]),
